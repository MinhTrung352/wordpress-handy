var AT_Main = {
	...
	,homeSlideshow : function(){
  	...
  	,homeIE : function(){
        if(jQuery('.home-slideshow-wrapper').length){
            jQuery('.home-slideshow-wrapper').each(function(index,value){
                var swiper = new Swiper('.swiper-container', {
                    autoplay: jQuery(value).data('sec')
                  	,pagination: '.swiper-pagination'
                    ,paginationClickable: '.swiper-pagination'
                    ,nextButton: '.swiper-button-next'
                    ,prevButton: '.swiper-button-prev'
	...
	,refreshZoom : function(){ // Update Zoom in detail page
  	...
  	,scareName : function(){
        if( jQuery('.cata-product.cp-grid').length > 0 ){
          	var _name_height = 0;
            jQuery('.cata-product.cp-grid').find('h5.product-name').each(function( index,value ){
                _name_height = jQuery(value).height() > _name_height ? jQuery(value).height() : _name_height;
            });
          	jQuery('.cata-product.cp-grid').find('h5.product-name').css('height',_name_height);
        }
    }          
    ,scareImage : function(){
